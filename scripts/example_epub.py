...
def get_paragraphs(book):
    ...
    paragraphs = list(soup.find_all(["p", "span"]))
    return paragraphs


def mapping(comm, paragraphs):
    """[A helper function for create_mapping. It maps each line in an ancient Greek literature to its corresponding commentary.]
    ...
    for para in paragraphs:
        ...
        if para.name == "p":
            ...
            if start:
                ...
                # Book Number
                if re.search(r"BOOK\sOF\sTHE\sILIAD.\s$", text):
                    curr_book = book_dict[text.split()[0]]

                # Section Comm (Eg: 1-7. ) and Edge Case (Eg: 28-32 = 11-15)
                elif re.search(
                    r"^\d{1,3}-\d{1,3}\s=\s\d{1,3}-\d{1,3}", text
                ) or re.search(r"^\d{1,3}-\d{1,3}.\s", text):
                    curr_sec_comm = text

                # Line Comm Edge Case (Eg: 324 = 137, 451 f. = 37 f . )
                elif re.search(r"^\d{1,3}\s=\s\d{1,3}", text) or (
                    re.search(r"^\d{1,3}\sf.\s=", text)
                ):
                    line_num = int(re.search(r"^\d+. ", text).group()[:-1])
                    comm.single_line_comment.append(
                        SingleLineComment(
                            book_number=curr_book,
                ...
                # Line Comm (Eg: 23. )
                elif re.search(r"^\d{1,3}.\s", text):
                    line_num = int(re.search(r"^\d{1,3}.\s", text).group()[:-2])
                    comm.single_line_comment.append(
                        SingleLineComment(
                            book_number=curr_book,
                            line_number=line_num,
...
def create_mapping(epub_path: str) -> Commentary:
    ...
    comm = Commentary(
        modern_author=book.get_metadata("DC", "creator")[1][0],
...
def query(epub_path: str, book_num: int, line_num: int, sec_comm: bool = False) -> str:
    ...
    """
    commentary: Commentary = create_mapping(epub_path)
    line_comment: str = commentary.query_line_comm(book_num, line_num, sec_comm)
    return line_comment


if __name__ == "__main__":
    FILE_NAME: str = (
        "../example-texts/firstthreebooks03homegoog_firstthreebooks03homegoog.epub"
    )
    BOOK_NUM: int = 1
    LINE_NUM: int = 2
    RES = query(FILE_NAME, BOOK_NUM, LINE_NUM)
    print(RES)
    RES = query(FILE_NAME, BOOK_NUM, LINE_NUM, sec_comm=True)
    print(RES)
